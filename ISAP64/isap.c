...
int isap_dec(const unsigned char *key, const unsigned char *nonce,
             const unsigned char *ciphertext, unsigned long long ciphertext_len,
             const unsigned char *mac, const unsigned char *ad, unsigned long long ad_len,
             unsigned char *plaintext) {
  unsigned char computed_mac[CRYPTO_ABYTES];
...
int main()
{
    ...
    if (isap_dec(key, nonce, ciphertext, ciphertext_len, tag,ad,ad_len, decrypted) == 0) {
        printf("Pesan tidak dapat didekripsi.\n");
    }
    else {
